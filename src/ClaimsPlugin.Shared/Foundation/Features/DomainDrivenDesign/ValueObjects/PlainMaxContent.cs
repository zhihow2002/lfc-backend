...
public class PlainMaxContent : BaseValueObject
{
    ...
    public static PlainMaxContent Create(string plainContent)
    {
        ...
        if (
            plainContent.HasLengthMoreThan(15000, out string? plainContentMaximumLengthErrorMessage)
        )
        {
        ...
        if (plainContent.IsNotRegex("/^[\\w!@#$%^&*()_-\\+=:;'\"|\\/,.<>? ]*$/"))
        {
            throw new DomainException(
                "Content accepts only alphabets [a-z, A-Z], numbers [0-9], spaces, and characters [!@#$%^&*()_-+=:;'\"|/,.<>?]."
            );
        }
